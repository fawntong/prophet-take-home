...
export const UserAvatar: React.FC<Props> = ({userName}) => {
    ...
    const initialsTruncated = initials.length > 2 ? [initials[0], initials[initials.length - 1]] : initials;
...
const AVATAR_COLORS: AvatarProps["color"][] = [
    "amber", "orange", "tomato", "crimson", "indigo", "cyan", "grass"
]
...
const generateAvatarColor = (userName: string) => {
    ...
    console.log(Math.abs(hash), AVATAR_COLORS.length, Math.abs(hash) % AVATAR_COLORS.length)
