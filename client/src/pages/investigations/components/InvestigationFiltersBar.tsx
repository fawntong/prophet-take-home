...
export const InvestigationFiltersBar: React.FC<Props> = ({activeFilters, updateFilters}) => {
    ...
    return (
        <div className="flex justify-between items-center">
            <div className="flex gap-2">
                {Object.entries(activeFilters).map(([filter, value]) => value ? <FilterBadge key={filter} filter={filter as Filter} filterValue={value} setFilter={setFilter} removeFilter={removeFilter}/> : <></>)}
                {pendingFilter && <FilterBadge defaultOpen={true}
                                               // Cancel the pending filter if it's been unfocused
                                               onOpenChange={() => setPendingFilter(null)} filter={pendingFilter} setFilter={setFilter} removeFilter={removeFilter}/>}
            </div>
            <AddFilterButton activeFilters={activeFilters} setPendingFilter={setPendingFilter}/>
        </div>
    )
}
...
// TODO: include times?
const FILTERABLE_COLUMNS: Record<Filter, {prettyName: string; options: [string, string][] | null}> = {
    determination : {
    ...
    return (
        <DropdownMenu.Root>
            <DropdownMenu.Trigger>
                {/* TODO: fix cursor */}
                <Button><MixerHorizontalIcon /> Filter <DropdownMenu.TriggerIcon /></Button>
            </DropdownMenu.Trigger>
            <DropdownMenu.Content align="end">
                {Object.entries(FILTERABLE_COLUMNS).map(([key, {prettyName}]) => {
                    ...
                    return(
                    <DropdownMenu.Item key={key} disabled={active} onSelect={() => setPendingFilter(key as Filter)}>{prettyName}</DropdownMenu.Item>
                )})}
...
}> = ({filter, filterValue, setFilter, removeFilter, defaultOpen, onOpenChange}) => {
                ...
                {options && (
                    <>
                        ...
                        {options.map(([value, label]) => (
                            <DropdownMenu.CheckboxItem key={value} checked={filterValue === label} onClick={() => setFilter(filter, label)}>
                                {label}
